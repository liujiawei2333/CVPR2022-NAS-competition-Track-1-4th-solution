...
class SuperNetDependFC(MyNetwork):
    def __init__(self, n_class, mode,  dynamic_resolution, act_func="relu", bn_param=(0., 1e-5)):
        ...
        self.classifier_512 = nn.Linear(512, self.n_class, bias=True)#expand_ratio=1
        self.classifier_488 = nn.Linear(488, self.n_class, bias=True)#expand_ratio=0.95
        self.classifier_464 = nn.Linear(464, self.n_class, bias=True)#expand_ratio=0.9
        self.classifier_432 = nn.Linear(432, self.n_class, bias=True)#expand_ratio=0.85
        self.classifier_408 = nn.Linear(408, self.n_class, bias=True)#expand_ratio=0.8
        self.classifier_384 = nn.Linear(384, self.n_class, bias=True)#expand_ratio=0.75
        self.classifier_360 = nn.Linear(360, self.n_class, bias=True)#expand_ratio=0.7
    ...
    def forward(self, x):
        ...
        if x.shape[1] == 512:#expand_ratio=1
            x = self.classifier_512(x)
        elif x.shape[1] == 488:#expand_ratio=0.95
            x = self.classifier_488(x)
        elif x.shape[1] == 464:#expand_ratio=0.9
            x = self.classifier_464(x)
        elif x.shape[1] == 432:#expand_ratio=0.85
            x = self.classifier_432(x)
        elif x.shape[1] == 408:#expand_ratio=0.8
            x = self.classifier_408(x)
        elif x.shape[1] == 384:#expand_ratio=0.75
            x = self.classifier_384(x)
        elif x.shape[1] == 360:#expand_ratio=0.7
            x = self.classifier_360(x)
        return x
    ...
    def get_active_subnet(self, preserve_weight=True):
        with torch.no_grad():
            ...
            if input_channel == 512:
                classifier = self.classifier_512
            elif input_channel == 488:
                classifier = self.classifier_488
            elif input_channel == 464:
                classifier = self.classifier_464
            elif input_channel == 432:
                classifier = self.classifier_432
            elif input_channel == 408:
                classifier = self.classifier_408
            elif input_channel == 384:
                classifier = self.classifier_384
            elif input_channel == 360:
                classifier = self.classifier_360
