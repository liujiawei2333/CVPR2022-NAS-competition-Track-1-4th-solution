...
class MBInvertedConvLayer(MyModule):
    ...
    def __init__(self, in_channels, out_channels,
                kernel_size=3, stride=1, expand_ratio=6, mid_channels=None, act_func='relu', use_se=False,
...
class BasicBlock(MyModule):
    ...
    def __init__(self, in_channel, middle_channel, out_channel, stride=1, act_func='relu6'):
        ...
        self.conv1 = nn.Conv2d(self.in_channel, self.middle_channel, 3, self.stride, 1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.middle_channel)
        self.act = build_activation(self.act_func, inplace=True)

        self.conv2 = nn.Conv2d(self.middle_channel, self.out_channel, 3, 1, 1, bias=False)
        self.bn2 = nn.BatchNorm2d(self.out_channel)

        self.short_conv = nn.Conv2d(self.in_channel, self.out_channel, 1, self.stride, 0, bias=False)
        self.short_bn = nn.BatchNorm2d(self.out_channel)

    def forward(self, x):
        identity_x = x
        in_channel = x.size(1)

        if in_channel != self.out_channel or self.stride != 1:
    ...
    @property
    def module_str(self):
        export_str = "middle channel: {}, out channel:{}".format(self.middle_channel, self.out_channel)
        return export_str
